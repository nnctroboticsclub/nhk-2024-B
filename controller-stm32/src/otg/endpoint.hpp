...
struct Endpoint {
  ...
  int SendPacket_1(const uint8_t* pbuff, const int length,
                   const bool setup_packet = false, const bool do_ping = false);
  HCD_URBStateTypeDef ReceivePacket_1(uint8_t* pbuff, const int length);
 ...
 protected:
  ...
  int SendSetupPacket(uint8_t* pbuff, int tx_length) {
    return SendPacket(pbuff, tx_length, true, true);
  }

  HCD_URBStateTypeDef SendDataIn(uint8_t* pbuff, int rx_length) {
    return ReceivePacket(pbuff, rx_length);
  }

  int SendDataOut(uint8_t* pbuff, int tx_length) {
...
class EndpointControl : public Endpoint {
 public:
  ...
  // For Out: Request
  // For In: Response
  HCD_URBStateTypeDef ControlRead(uint8_t* pbuff, int tx_len, const int rx_len);

  HCD_URBStateTypeDef ControlWrite(uint8_t* setup_data, int setup_len,
