...
class HC::Impl {
 ...
 public:
  ...
  ~Impl() {
    ...
    while (thread_running_mark_) robotics::system::SleepFor(1ms);

    if (log_verbose_enabled) {
      logger.Trace(">>>>>> Release channel %d", slot_.GetChannel());
      logger.Trace("\x1b[32m----------------------------------------\x1b[0m");
    }
  ...
  void Init(int ep_addr, int dev_addr, int ep_type, int max_packet_size) {
    ...
    if (0)
      logger.Info("ini: HC[%d]: ep%d, dev%d, speed%d, type%d, size%d",
  ...
  void SubmitRequest(int direction, uint8_t *pbuff, int length, bool setup,
    ...
    // hc_->toggle_in, hc_->toggle_out is not used by usb ll driver
    // (its used by usb hal driver)

    if (0)
      logger.Info("req: data_pid%d, toggle[in,out]=[%d,%d] dir%d ping%d",
