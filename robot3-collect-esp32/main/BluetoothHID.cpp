...
extern "C" void CBluetoothHID::hidh_callback(void *handler_args,
    ...
    case ESP_HIDH_OPEN_EVENT: {
      ...
      if (!pDevice) {
        for (size_t i = 0; i < BluetoothHID.m_BluetoothDevicesSize; i++) {
          ...
          if (pDev) {
            // if (pDev->pbtDev->m_PID == param->open.dev->config.product_id &&
            //     pDev->pbtDev->m_VID == param->open.dev->config.vendor_id) {
