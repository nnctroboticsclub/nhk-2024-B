...
class CBluetoothDeviceBase : IBluetoothDevice {
 ...
 public:
  ...
  BTHIDState GetState() { return m_State; }

 protected:
  esp_hidh_dev_t *dev() { return m_pDev; }
 ...
 private:
  ...
  void EventHandler(void *handler_args, esp_event_base_t base, int32_t id,
                    void *event_data) {
    ...
    switch (event) {
      case ESP_HIDH_OPEN_EVENT: {
        ...
        ESP_LOGI(BTHID_TAG, ESP_BD_ADDR_STR " OPEN: %s", ESP_BD_ADDR_HEX(bda),
      ...
      case ESP_HIDH_BATTERY_EVENT: {
        ...
        ESP_LOGI(BTHID_TAG, ESP_BD_ADDR_STR " BATTERY: %d%%",
      ...
      case ESP_HIDH_INPUT_EVENT: {
        ...
        ESP_LOGI(BTHID_TAG, ESP_BD_ADDR_STR " INPUT: %8s, MAP: %2u, ID: %3u,
        Len: %d, Data:", ESP_BD_ADDR_HEX(bda),
        esp_hid_usage_str(param->input.usage), param->input.map_index,
        ...
        memcpy((void *)&InputReport,
               (void *)((uint8_t *)param + sizeof(void *)),
               sizeof(InputReport));
      ...
      case ESP_HIDH_FEATURE_EVENT: {
        ...
        ESP_LOGI(BTHID_TAG, ESP_BD_ADDR_STR " FEATURE: %8s, MAP: %2u, ID: %3u,
        Len: %d", ESP_BD_ADDR_HEX(bda), esp_hid_usage_str(param->feature.usage),
        param->feature.map_index, param->feature.report_id,
        param->feature.length); ESP_LOG_BUFFER_HEX(BTHID_TAG,
        param->feature.data, param->feature.length);*/

        BHIDFeatureEvt FeatureReport;
        memcpy((void *)&FeatureReport,
               (void *)((uint8_t *)param + sizeof(void *)),
      ...
      case ESP_HIDH_CLOSE_EVENT: {
        ...
        ESP_LOGI(BTHID_TAG, ESP_BD_ADDR_STR " CLOSE: '%s' %s",
        ESP_BD_ADDR_HEX(bda), esp_hidh_dev_name_get(param->close.dev),
        esp_hid_disconnect_reason_str(esp_hidh_dev_transport_get(param->close.dev),
        param->close.reason));
        */
...
class CBluetoothHID {
 public:
  ...
  void ScanAndConnectHIDDevice() {
    xTaskCreate(&hid_device_search, "hid_device_search", 4 * 1024, nullptr, 2,
                nullptr);
  }
  ...
  bool DegisterDevice(CBluetoothDeviceBase *pDevice) {
    ...
    for (; i < m_BluetoothDevicesSize; i++) {
      ...
      m_pBluetoothDevices[i + 1]->pbtDev = nullptr;
    }
...
#define SCAN_DURATION_SECONDS 5
  ...
  static void hid_device_search(void *pvParameters) {
    ...
    if (results_len) {
      ...
      while (r) {
        /*printf("  %s: " ESP_BD_ADDR_STR ", ", (r->transport ==
      ESP_HID_TRANSPORT_BLE) ? "BLE" : "BT ", ESP_BD_ADDR_HEX(r->bda));
      ...
      if (cr) {
        ...
        // ESP_HID_TRANSPORT_BLE) ? "BLE" : "BT ", ESP_BD_ADDR_HEX(cr->bda),
        // cr->name ? cr->name : "");
        // esp_hidh_dev_t *p_dev =
