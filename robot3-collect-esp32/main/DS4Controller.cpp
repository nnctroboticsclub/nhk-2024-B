...
const uint8_t OutputReportDefault[] = {
    0xc0, 0x20, 0xf0, 0x04, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x43, 0x43,
    0x00, 0x4d, 0x85, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xd8, 0x8e, 0x94, 0xdd};
...
void DS4Controller::OnOpenDevice() { ESP_LOGD(TAG, "%s", __FUNCTION__); }
...
void DS4Controller::OnInputReport(BHIDInputEvt *pInputReport) {
  ...
  // Extended report
    ...
    crc = ~this->crc32_le(crc, pInputReport->data,
                          pInputReport->length - sizeof(uint32_t));
    ...
    if (crc == ((DS4DataExt *)pInputReport->data)->crc32) {
      ...
      ESP_LOGW(TAG, "CRC32 NG! 0x%08lX != 0x%08lX", crc,
               ((DS4DataExt *)pInputReport->data)->crc32);
    }
  }
}
...
void DS4Controller::SetExtendedReporting() {
  esp_hidh_dev_feature_get(this->dev(), 0, 0x02, DS4_FEATURE_REPORT_0x02_SIZE,
                           (uint8_t *)&ds4feature02, &ds4feature02len);
}
...
void DS4Controller::SetRumble(uint8_t left, uint8_t right, bool update) {
  ...
  m_pOutputReport[DS4_OUTPUT_UPDATE_CONTROL_INDEX] |=
      (DS4_OUTPUT_UPDATE_RUMBLE & DS4_OUTPUT_UPDATE_MASK);
...
void DS4Controller::SetLED(uint8_t r, uint8_t g, uint8_t b, bool update) {
  if (DS4LEDColor.r != r || DS4LEDColor.g != g || DS4LEDColor.b != b) {
    ...
    m_pOutputReport[DS4_OUTPUT_UPDATE_CONTROL_INDEX] |=
        (DS4_OUTPUT_UPDATE_LED & DS4_OUTPUT_UPDATE_MASK);
...
void DS4Controller::UpdateState() { SendOutputReport(); }

// esp_hidh_dev_output_set(esp_hidh_dev_t *dev, size_t map_index, size_t
// report_id, uint8_t *data, size_t length);
void DS4Controller::SendOutputReport() {
  ...
  crc = this->crc32_le(0xFFFFFFFF, (uint8_t *)&bthdr, sizeof(bthdr));
  crc = ~this->crc32_le(crc, m_pOutputReport,
                        sizeof(OutputReportDefault) - sizeof(uint32_t));

  *((uint32_t *)(m_pOutputReport + DS4_OUTPUT_LED_CRC_INDEX)) = crc;

  esp_hidh_dev_output_set(this->dev(), 0, 0x11, m_pOutputReport,
                          sizeof(OutputReportDefault));
  // Clear control bits
...
uint32_t DS4Controller::crc32_le(unsigned int crc, unsigned char const *buf,
                                 unsigned int len) {
  uint32_t i;
  for (i = 0; i < len; i++) {
