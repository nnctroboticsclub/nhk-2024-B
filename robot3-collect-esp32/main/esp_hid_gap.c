...
#if CONFIG_BT_BLE_ENABLED
static void handle_ble_device_result(
  ...
  uint8_t *uuid_d = esp_ble_resolve_adv_data(
      scan_rst->ble_adv, ESP_BLE_AD_TYPE_16SRV_CMPL, &uuid_len);
  if (uuid_d != NULL && uuid_len) {
    uuid = uuid_d[0] + (uuid_d[1] << 8);
  }

  uint8_t appearance_len = 0;
  uint8_t *appearance_d = esp_ble_resolve_adv_data(
      scan_rst->ble_adv, ESP_BLE_AD_TYPE_APPEARANCE, &appearance_len);
  ...
  uint8_t *adv_name = esp_ble_resolve_adv_data(
      scan_rst->ble_adv, ESP_BLE_AD_TYPE_NAME_CMPL, &adv_name_len);

  if (adv_name == NULL) {
    adv_name = esp_ble_resolve_adv_data(
        scan_rst->ble_adv, ESP_BLE_AD_TYPE_NAME_SHORT, &adv_name_len);
  }
...
static void bt_gap_event_handler(esp_bt_gap_cb_event_t event,
  ...
  switch (event) {
    ...
    case ESP_BT_GAP_KEY_NOTIF_EVT:
      ESP_LOGI(TAG, "BT GAP KEY_NOTIF passkey:%" PRIu32,
               param->key_notif.passkey);
      break;
    case ESP_BT_GAP_CFM_REQ_EVT: {
      ESP_LOGI(TAG,
               "BT GAP CFM_REQ_EVT Please compare the numeric value: %" PRIu32,
...
static void ble_gap_event_handler(esp_gap_ble_cb_event_t event,
  ...
  switch (event) {
    ...
    case ESP_GAP_BLE_KEY_EVT:  // shows the ble key info share with peer device
                               // to the user.
    ...
    case ESP_GAP_BLE_PASSKEY_NOTIF_EVT:  // ESP_IO_CAP_OUT
      ...
      // peer device IO has Input capability. Show the passkey number to the
      // user to input it in the peer device.
    ...
    case ESP_GAP_BLE_PASSKEY_REQ_EVT:  // ESP_IO_CAP_IN
      ...
      // peer device IO has Output capability. See the passkey number on the
      // peer device and send it back.
    ...
    case ESP_GAP_BLE_SEC_REQ_EVT:
      ...
      // the security request. If not accept the security request, should send
      // the security response with negative(false) accept value.
