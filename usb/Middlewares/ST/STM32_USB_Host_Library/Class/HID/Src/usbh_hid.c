...
static USBH_StatusTypeDef USBH_HID_InterfaceInit(USBH_HandleTypeDef *phost) {
  ...
  interface =
      USBH_FindInterface(phost, phost->pActiveClass->ClassCode, 0xFF, 0xFFU);

  if ((interface == 0xFFU) ||
      (interface >= USBH_MAX_NUM_INTERFACES)) /* No Valid Interface */
  {
  ...
  if (phost->device.CfgDesc.Itf_Desc[interface].bInterfaceProtocol ==
    ...
    USBH_UsrLog("Raw device(Unknown) found!");
    HID_Handle->Init = USBH_HID_RawInit;
  }
  ...
  for (num = 0U; num < max_ep; num++) {
    if ((phost->device.CfgDesc.Itf_Desc[interface]
             ...
             .bEndpointAddress &
         0x80U) != 0U) {
      HID_Handle->InEp = (phost->device.CfgDesc.Itf_Desc[interface]
                              .Ep_Desc[num]
                              .bEndpointAddress);
      ...
      HID_Handle->OutEp = (phost->device.CfgDesc.Itf_Desc[interface]
                               .Ep_Desc[num]
                               .bEndpointAddress);
...
static USBH_StatusTypeDef USBH_HID_ClassRequest(USBH_HandleTypeDef *phost) {
  ...
  switch (HID_Handle->ctl_state) {
    ...
    case USBH_HID_REQ_GET_REPORT_DESC:
      ...
      /* Get Report Desc */
      classReqStatus = USBH_HID_GetHIDReportDescriptor(
          phost, HID_Handle->HID_Desc.wItemLength);
    ...
    case USBH_HID_REQ_SET_PROTOCOL:
      ...
      /* set protocol */
      classReqStatus = USBH_HID_SetProtocol(phost, 1U);  // Report Protocol
      if (classReqStatus == USBH_OK) {
        HID_Handle->ctl_state = USBH_HID_REQ_IDLE;
...
USBH_StatusTypeDef USBH_HID_GetHIDReportDescriptor(USBH_HandleTypeDef *phost,
  ...
  if (length > sizeof(phost->device.Data)) {
    USBH_ErrLog(
        "Control error: Get HID Report Descriptor failed, data buffer size "
        "issue");
...
USBH_StatusTypeDef USBH_HID_SetIdle(USBH_HandleTypeDef *phost, uint8_t duration,
  ...
  phost->Control.setup.b.wValue.w =
      (uint16_t)(((uint32_t)duration << 8U) | (uint32_t)reportId);
...
USBH_StatusTypeDef USBH_HID_SetReport(USBH_HandleTypeDef *phost,
  ...
  phost->Control.setup.b.wValue.w =
      (uint16_t)(((uint32_t)reportType << 8U) | (uint32_t)reportId);
